...
# rofi params
rofi_prompt="Search"
rofi_mesg="<b>Enter</b> open file | <b>Alt+C</b> copy path"
...
format_filename() {
    while read -r path; do
        if [ -f "$path" ]; then
            if [[ -n "$ROFI_ICONS" || $search_type = "Pictures" || $search_type = "Videos" ]]; then
...
menu_folder() {
    ...
    while true; do
        selected=$(search_folder "$folder" "$regex" | \
            ...
            $rofi_search_flags \
            $rofi_shortcuts \
            -config ~/.config/rofi/basiceffect.rasi \
        ...
        [[ -n "$ROFI_ICONS" || "$search_type" =~ (Pictures|Videos) ]] && selected=$(compose_filename "$selected")
...
init_search() {
    if [ -n "$search_type" ]; then
        case "$search_type" in
            "All") regex=".*" ;;
            "Books") regex=".*\.\(djvu\|epub\|mobi\|cbr\)" ;;
            "Content") file_contents="1" ;;
            "Desktop") path="$HOME/Desktop" ;;
            "Documents") regex=".*\.\(pdf\|txt\|md\|csv\|xlsx\|doc\|docx\)" ;;
            "Downloads") path="$HOME/Downloads" ;;
            "Music") regex=".*\.\(mp3\|wav\|m3u\|acc\|flac\|ogg\)" ;;
            "Pictures")
                rofi_search_flags="-show-icons -eh 2 -sep | -markup-rows"
                rofi_theme="$rofi_theme_preview"
                regex=".*\.\(jpg\|jpeg\|png\|tif\|tiff\|nef\|raw\|dng\|webp\|bmp\|xcf\)" ;;
            "Recent") recent_files="1" ;;
            "Videos")
                rofi_search_flags="-show-icons -eh 2 -sep | -markup-rows"
                rofi_theme="$rofi_theme_preview"
                regex=".*\.\(mkv\|avi\|mp4\|mov\|webm\|yuv\|mpg\|mpeg\|m4v\)" ;;
            *) echo "Invalid search type: $search_type"; print_help && exit 1 ;;
        esac
    fi
}
...
Videos\x00icon\x1ffolder-videos"
    ...
    while choice=$(echo -en "$search_entries" | \
        ...
        [ -z "$ROFI_ICONS" ] && { rofi_theme=""; rofi_search_flags=""; }
...
while getopts ":hHmRr:p:q:t:v" option; do
    case $option in
        ...
        H) skip_hidden="" ;;
        m) show_menu="1" ;;
        p) initial_path="$OPTARG"; path="$initial_path" ;;
        q) file_contents="1"; query="$OPTARG" ;;
        r) initial_regex="$OPTARG"; regex="$initial_regex" ;;
        R) recent_files="1" ;;
