...
sudo pacman -S fastfetch firefox kitty powerline-fonts starship flatpak rsync ttf-firacode-nerd ttf-meslo-nerd ttf-roboto awesome-terminal-fonts terminus-font ttf-droid --noconfirm --needed
...
#sudo grub-mkconfig -o /boot/grub/grub.cfg
#sudo pacman -S polkit-gnome unzip htop jq xfce4-terminal xfce4-taskmanager imagemagick thunar thunar-volman thunar-archive-plugin gvfs lxappearance && mkdir -p ~/.themes && tar -xvf ~/Downloads/inst/script/Material-Black-Blueberry-2.9.9-07.tar -C ~/.themes > /dev/null && mkdir -p ~/.icons && unzip ~/Downloads/inst/script/MB-Blueberry-Suru-GLOW_1.9.3.zip -d ~/.icons > /dev/null
#xfce4-terminal --drop-down /etc/lightdm/lightdm.conf or /etc/sddm.conf [Autologin] User=howling Session=i3
