...
sudo pacman -S git base-devel --noconfirm --needed && git clone https://aur.archlinux.org/yay-bin.git && cd ~/Downloads/inst/yay-bin && makepkg --noconfirm -si && cd ~/Downloads/inst && rm -rf yay-bin && yay -Syu --noconfirm && yay -Y --sudoloop --save
sudo pacman -S brightnessctl inotify-tools btop wireplumber playerctl bash-completion neovim trash-cli xclip pacman-contrib jq gnome-system-monitor reflector eza zoxide fzf bat feh zip unzip ripgrep tree gdu less tldr --noconfirm --needed && cp -rf ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -rf ~/Downloads/inst/files/* /root/.config/
...
if pacman -Qs plasma-desktop >/dev/null; then
    ...
    sudo pacman -S --noconfirm --needed xorg-xkill xfburn mousepad xorg-xev i3-wm polybar python-i3ipc autotiling i3lock wmctrl xorg-xprop thunar gvfs gvfs-mtp gvfs-afc htop polkit-gnome xorg-xinput network-manager-applet lxappearance && chmod +x ~/.config/polybar/launch.sh
...
sudo pacman -S kitty powerline-fonts starship flatpak rsync ttf-firacode-nerd ttf-meslo-nerd ttf-roboto terminus-font noto-fonts-emoji ttf-nerd-fonts-symbols --noconfirm --needed
#----Swap-------
#sudo sh -c 'pacman -S zram-generator --noconfirm --needed && mkdir -p /etc/systemd/ && printf "[zram0]\nzram-size=ram/2\ncompression-algorithm=lz4\nswap-priority=100\n" > /etc/systemd/zram-generator.conf && systemctl daemon-reload && systemctl start systemd-zram-setup@zram0.service && systemctl enable systemd-zram-setup@zram0.service'
...
#-----------------------------------------------------
sudo pacman -S procs magic-wormhole warp binutils nmap gcc patch fakeroot bind yazi gimp mediainfo-gui mediainfo npm xournalpp torbrowser-launcher --noconfirm --needed
...
#-----------------------------------------------------------------
yay -S --noconfirm --needed dxvk-bin ventoy-bin winegui-bin ttf-ms-fonts dragon-drop urlview autobrr-bin reflector-simple surge-bin
...
#/etc/lightdm/lightdm-gtk-greeter.conf /etc/lightdm/lightdm.conf or /etc/sddm.conf [Autologin] User=$(whoami) Session=i3   other: pika-backup   
...
#wayland = xdg-desktop-portal-wlr egl-wayland niri qt5ct  qt6ct  nwg-look nwg-displays xdg-desktop-portal xdg-desktop-portal-gtk slurp wlogout swaybg waybar wl-clipboard kvantum xdg-user-dirs libspng python-requests python-pyquery qt6-svg xdg-utils qt5-wayland qt6-wayland kvantum-qt5 wev grim wtype xwayland-satellite swaylock
#sudo pacman -S --noconfirm --needed egl-wayland hyprland hyprlock polkit-gnome pipewire pipewire-alsa pipewire-audio pipewire-jack pipewire-pulse sof-firmware gst-plugin-pipewire wireplumber pavucontrol pamixer networkmanager network-manager-applet bluez bluez-utils blueman brightnessctl udiskie qt5ct  qt6ct  nwg-look nwg-displays xdg-desktop-portal-hyprland xdg-desktop-portal-gtk rofi-wayland slurp wlogout swaync swaybg waybar wl-clipboard thunar thunar-volman tumbler ffmpegthumbnailer thunar-archive-plugin xarchiver gvfs gvfs-mtp gvfs-afc catfish kvantum playerctl brightnessctl xdg-user-dirs libspng python-requests python-pyquery loupe qt6-svg xdg-utils qt5-wayland qt6-wayland kvantum-qt5 wev grim slurp wl-clipboard xdg-utils wtype
