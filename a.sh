...
chmod +x ~/Downloads/inst/scripts/* && mv ~/Downloads/inst/scripts/* ~/.local/bin/ && mkdir ~/.othercrap && mv ~/Downloads/inst/script/*.png ~/.othercrap/ && chmod +x ~/.config/hypr/scripts/* && chmod +x ~/.config/i3/scripts/*
...
if pacman -Qs plasma-desktop >/dev/null; then
    ...
    sudo pacman -S --noconfirm --needed i3-wm polybar python-i3ipc autotiling i3lock lxappearance
    sudo pacman -S --noconfirm --needed bc arandr viewnior rofi rofi-calc flameshot gvfs-mtp gvfs-afc mpv-mpris baobab numlockx tumbler thunar-archive-plugin file-roller fbreader ffmpegthumbnailer catfish pamixer dunst pavucontrol thunar-volman  
...
#-----------------------------------------------------------------
yay -S --noconfirm dxvk-bin ventoy-bin winegui-bin input-remapper-bin ttf-ms-fonts dragon-drop heroic-games-launcher-bin urlview pacseek-bin
