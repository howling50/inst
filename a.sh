...
sudo sh -c 'for option in "Color" "ILoveCandy" "VerbosePkgLists"; do grep -qx "$option" /etc/pacman.conf || sed -i "/\[options\]/a $option" /etc/pacman.conf; done' && sudo sed -i 's/^#Para/Para/' /etc/pacman.conf && sudo sed -i '/^\s*#\[multilib\]/s/^#//' /etc/pacman.conf && sudo sed -i '/^\s*#Include = \/etc\/pacman\.d\/mirrorlist/s/^#//' /etc/pacman.conf
...
sudo pacman -S xclip jq gnome-system-monitor file-roller reflector eza zoxide fzf bat feh zip unzip --noconfirm --needed && cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/
mkdir -p ~/.themes && tar -xvf ~/Downloads/inst/script/Material-Black-Blueberry-2.9.9-07.tar -C ~/.themes > /dev/null && mkdir -p ~/.icons && unzip ~/Downloads/inst/script/Material-Black-Blueberry-Numix_1.9.3.zip -d ~/.icons > /dev/null && gtk-update-icon-cache -f -t /home/howling/.icons/Material-Black-Blueberry-Numix/
...
sudo pacman -S binutils nmap gcc patch fakeroot bind rofi rofi-calc yazi gimp easytag mediainfo-gui mediainfo npm --noconfirm --needed
sudo pacman -S qbittorrent putty aria2 bluez bluez-utils blueman fuseiso android-tools mpv vlc libreoffice-fresh viewnior cava fbreader perl-image-exiftool shotcut hexchat gnome-boxes handbrake --noconfirm --needed
...
#-----------------------------------------------------------------
yay -S --noconfirm dxvk-bin ventoy-bin reflector-simple winegui-bin input-remapper-git ttf-ms-fonts dragon-drop
#yay -S --noconfirm konsave
flatpak install --noninteractive flathub com.heroicgameslauncher.hgl && flatpak install --noninteractive flathub io.gitlab.librewolf-community && flatpak install --noninteractive flathub com.github.tchx84.Flatseal && flatpak install --noninteractive flathub org.torproject.torbrowser-launcher && flatpak install --noninteractive flathub io.github.giantpinkrobots.varia
...
curl -sL https://www.rarlab.com/rar/rarlinux-x64-$(curl -sL https://www.rarlab.com/download.htm | grep -oP 'rarlinux-x64-\K[0-9]+\.tar\.gz' | head -n1) -o rarlinux.tar.gz && tar -xzf rarlinux.tar.gz && sudo mv rar/rar /usr/local/bin/ && rm -rf rar rarlinux.tar.gz
