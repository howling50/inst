...
sudo pacman -S btop wireplumber playerctl bash-completion neovim trash-cli xclip pacman-contrib jq gnome-system-monitor reflector eza zoxide fzf bat feh zip unzip --noconfirm --needed && cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/
chmod +x ~/Downloads/inst/scripts/* && mv ~/Downloads/inst/scripts/* ~/.local/bin/ && mkdir ~/.othercrap && chmod +x ~/.config/hypr/scripts/* && chmod +x ~/.config/i3/scripts/* && mv ~/Downloads/inst/script/wallpaper ~/.othercrap/ && unzip ~/Downloads/inst/script/1.zip -d ~/.othercrap > /dev/null
...
if pacman -Qs plasma-desktop >/dev/null; then
    ...
    sudo pacman -S --noconfirm --needed i3-wm polybar python-i3ipc autotiling i3lock lxappearance udiskie wmctrl xorg-xprop thunar xfce4-terminal xfce4-taskmanager htop polkit-gnome
...
#sudo pacman -S --noconfirm --needed hyprland  hyprlock hyprpolkitagent pipewire pipewire-alsa pipewire-audio pipewire-jack pipewire-pulse sof-firmware gst-plugin-pipewire wireplumber pavucontrol pamixer networkmanager network-manager-applet bluez bluez-utils blueman brightnessctl udiskie qt5ct  qt6ct  nwg-look nwg-displays xdg-desktop-portal-hyprland xdg-desktop-portal-gtk rofi-wayland slurp wlogout swaync swaybg waybar wl-clipboard thunar thunar-volman tumbler ffmpegthumbnailer thunar-archive-plugin xarchiver gvfs gvfs-mtp gvfs-afc catfish kvantum playerctl brightnessctl xdg-user-dirs libspng python-requests python-pyquery loupe qt6-svg xdg-utils qt5-wayland qt6-wayland kvantum-qt5 && yay -S --noconfirm --needed grimblast-git
#sudo pacman -Rns lightdm lightdm-gtk-greeter && sudo systemctl stop lightdm && sudo systemctl disable lightdm && sudo pacman -S --noconfirm --needed  sddm qt6-5compat qt6-declarative qt6-svg && sudo systemctl enable sddm && sudo mkdir -p /etc/sddm.conf.d && echo -e "[Theme]\nCurrent = sequoia" | sudo tee "/etc/sddm.conf.d/theme.conf.user" > /dev/null && sudo cp -rf "$HOME/.othercrap/wallpaper/monkey.png" "/usr/share/sddm/themes/sequoia/backgrounds/default" 2>&1 && sudo sed -i 's|^wallpaper=".*"|wallpaper="backgrounds/default"|' "/usr/share/sddm/themes/sequoia/theme.conf" 2>&1
