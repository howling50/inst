...
chmod +x ~/Downloads/inst/scripts/* && mv ~/Downloads/inst/scripts/* ~/.local/bin/ && mkdir ~/.othercrap && chmod +x ~/.config/hypr/scripts/* && chmod +x ~/.config/i3/scripts/* && mv ~/Downloads/inst/script/wallpaper ~/Pictures/ && unzip -o ~/Downloads/inst/script/1.zip -d ~/.othercrap > /dev/null
...
#sudo pacman -S --noconfirm --needed egl-wayland hyprland hyprlock polkit-gnome pipewire pipewire-alsa pipewire-audio pipewire-jack pipewire-pulse sof-firmware gst-plugin-pipewire wireplumber pavucontrol pamixer networkmanager network-manager-applet bluez bluez-utils blueman brightnessctl udiskie qt5ct  qt6ct  nwg-look nwg-displays xdg-desktop-portal-hyprland xdg-desktop-portal-gtk rofi-wayland slurp wlogout swaync swaybg waybar wl-clipboard thunar thunar-volman tumbler ffmpegthumbnailer thunar-archive-plugin xarchiver gvfs gvfs-mtp gvfs-afc catfish kvantum playerctl brightnessctl xdg-user-dirs libspng python-requests python-pyquery loupe qt6-svg xdg-utils qt5-wayland qt6-wayland kvantum-qt5 wev grim slurp wl-clipboard swappy xdg-utils wtype
