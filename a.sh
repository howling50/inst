...
sudo pacman -S file-roller reflector eza zoxide fzf bat feh --noconfirm --needed && cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/
...
sudo curl -sL "https://raw.githubusercontent.com/Benexl/yt-x/refs/heads/master/yt-x" -o /usr/local/bin/yt-x && sudo chmod +x /usr/local/bin/yt-x && sudo mkdir -p /usr/lib/vlc/lua/extensions/ && sudo mv ~/Downloads/inst/script/VlcPrevNext.lua /usr/lib/vlc/lua/extensions/
