...
# Set error handling
set -eo pipefail
trap 'echo "Error occurred at line $LINENO. Check error.log for more details."' ERR
...
cd
cd ~/Downloads/inst/
curl -O https://blackarch.org/strap.sh || exit
chmod +x strap.sh
sudo ./strap.sh
...
sudo rm -rf ~/.zshrc || exit
sudo rm -rf ~/.bashrc || exit
...
balooctl disable
sudo rm -rf ~/./local/share/baloo || exit
