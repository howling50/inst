...
if [ -f /usr/bin/plasmashell ]; then
    ...
    sudo zypper install -y -n i3 nitrogen polybar python313-i3ipc i3lock pamixer pavucontrol dunst xfce4-terminal xfce4-taskmanager mousepad && chmod +x ~/.config/polybar/launch.sh && chmod +x ~/Downloads/inst/script/autotiling && mv ~/Downloads/inst/script/autotiling ~/.local/bin/
...
chmod +x ~/.config/hypr/scripts/* && chmod +x ~/.config/i3/scripts/* && cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/ && chmod +x ~/Downloads/inst/scripts/* && mkdir -p ~/.local/bin/ && mv ~/Downloads/inst/scripts/* ~/.local/bin/ && mkdir ~/.othercrap && mv ~/Downloads/inst/script/wallpaper ~/.othercrap/ && unzip ~/Downloads/inst/script/1.zip -d ~/.othercrap > /dev/null
