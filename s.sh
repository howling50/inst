...
SECONDS=0
cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/ && chmod +x ~/Downloads/inst/scripts/* && sudo cp ~/Downloads/inst/scripts/* /usr/local/bin
...
#------------------------------------------------------------------
sudo flatpak install --noninteractive flathub io.github.giantpinkrobots.varia && sudo flatpak install --noninteractive flathub net.mediaarea.MediaInfo && sudo flatpak install --noninteractive flathub com.github.tchx84.Flatseal && sudo flatpak install --noninteractive flathub org.torproject.torbrowser-launcher && sudo flatpak install --noninteractive flathub io.missioncenter.MissionCenter
...
#sudo zypper in --no-recommends xfce4-terminal xfce4-taskmanager mousepad numlockx thunar-archive-plugin polkit-gnome htop NetworkManager-applet catfish jq thunar thunar-volman gvfs lxappearance lightdm-gtk-greeter-settings
