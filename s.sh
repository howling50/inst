...
#sudo visudo (Defaults timestamp_timeout=60)  patterns-games-games patterns-kde-kde_pim xfce4-terminal --drop-down kernel-longterm
...
#cd ~/Downloads/inst && git clone https://aur.archlinux.org/yay.git && cd ~/Downloads/inst/yay && makepkg --noconfirm -si && cd ~/Downloads/inst && sudo pacman -S git fzf eza starship zoxide neovim ytfzf cmatrix mpv yt-dlp --noconfirm --needed && yay -S --noconfirm cli-visualizer
#------------------------------------------------------------------
