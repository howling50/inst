...
#/etc/sysconfig/btrfsmaintenance /etc/snapper/configs/root   (about:profiles, open root profile folder,Clear start up cache" )
...
sudo zypper install -y -n kernel-longterm powerline-fonts starship memtest86+ kitty flatpak && sudo flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo
#----Swap-------
...
#sudo zypper install -y -n xfce4-panel-profiles xfce4-whiskermenu-plugin xfce4-screenshooter xfce4-taskmanager adwaita-icon-theme dmz-icon-theme-cursors && sudo zypper remove -y parole && sudo zypper addlock parole
