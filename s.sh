...
#sudo zypper install -y -n xhost && xhost +local: && sudo zypper in -y -n dnf rpm-repos-openSUSE-Tumbleweed && sudo sed -i '/^\[main\]$/a fastestmirror=True\nmax_parallel_downloads=10\ndeltarpm=True\ndefaultyes=true' /etc/dnf/dnf.conf
...
#sudo zypper in --no-recommends pavucontrol dunst xfce4-terminal xfce4-taskmanager mousepad numlockx thunar-archive-plugin polkit-gnome htop NetworkManager-applet catfish jq thunar thunar-volman gvfs lxappearance
