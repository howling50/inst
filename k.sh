...
if pacman -Qs plasma-desktop >/dev/null; then
    ...
    sudo pacman -S --noconfirm --needed i3-wm polybar python-i3ipc autotiling i3lock lxappearance wmctrl xorg-xprop
...
sudo pacman -S cpu-x gamemode gsmartcontrol cmatrix w3m ddgr cmus xdotool podman distrobox e2fsprogs ripgrep memtest86+ tree gdu less mpg123 imagemagick tldr feh alsa-utils gptfdisk ntfs-3g os-prober python-pip --noconfirm --needed
#-----------------------------------------------------------------
...
sudo touch  /root/.profile && sudo sed -i '/GPU_ENV_SET/d; /LIBVA_DRIVER_NAME/d' /root/.profile && sudo bash -c 'echo -e "\n# GPU detection\nif command -v lspci >/dev/null; then\n  if lspci | grep -iq \"nvidia\"; then\n    export LIBVA_DRIVER_NAME=nvidia\n    export VDPAU_DRIVER=nvidia\n    export __GLX_VENDOR_LIBRARY_NAME=nvidia\n  elif lspci | grep -iq \"amd\\|radeon\"; then\n    export LIBVA_DRIVER_NAME=radeonsi\n    export VDPAU_DRIVER=radeonsi\n    unset __GLX_VENDOR_LIBRARY_NAME\n  fi\n  export GPU_ENV_SET=1\nfi" >> /root/.profile'
#-----------------
