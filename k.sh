...
sudo pacman -S jq gnome-system-monitor file-roller eza zoxide fzf bat feh zip unzip --noconfirm --needed && cp -r ~/Downloads/inst/files/* ~/.config/ && sudo mkdir -p /root/.config && sudo cp -r ~/Downloads/inst/files/* /root/.config/
...
sudo pacman -S qbittorrent putty aria2 bluez bluez-utils blueman fuseiso android-tools mpv vlc libreoffice-fresh viewnior cava --noconfirm --needed
...
#-----------------------------------------------------------------
yay -S --noconfirm dxvk-bin winegui-bin input-remapper-git ttf-ms-fonts
